...
export const fetchPage = async (
	...
	try {
		const res = await axios.get<{ data: any[] }>(url, {
			headers: {
...
> = {
	...
	update: async ({ entity, items, jobData }) => {
		...
		console.log(
			`[update handler] entity=${entity}, update field=${field}, value=${JSON.stringify(value)}, items=${enrichedItems.map((i) => i.id).join(", ")}`,
		);
	...
	export: async ({ entity, items, jobData }) => {
...
export const startMassActionsWorker = () => {
	for (let i = 0; i < env.DAL_WORKER_COUNT; i++) {
		...
		const worker = new BullWorker<JobData>(
			"csvMassActionsQueue",
			async (job) => {
				const { entity, searchMask, mass_action, list_id } = job.data;
				logger.info(
