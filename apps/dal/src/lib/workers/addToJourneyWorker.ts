		...
		const worker = new BullWorker(
			...
			async (job) => {
				...
				for (let i = 0; i < contactIds.length; i += BATCH_SIZE) {
					...
					try {
						await pool.query(
							`DELETE FROM journey_steps_contacts_links
						...
						const res = await pool.query(
							`INSERT INTO journey_steps_contacts_links (journey_step_id, contact_id)
							 VALUES ${placeholders}
							 RETURNING contact_id`,
							values,
						);

						const insertedIds = res.rows.map((row: { contact_id: number }) => row.contact_id);
