				...
				for (let i = 0; i < contactIds.length; i += BATCH_SIZE) {
					...
					try {
						const result = await pool.query(
							`INSERT INTO contacts_lists_links (contact_id, list_id)
				   VALUES ${placeholders}
				   ...
				   RETURNING contact_id`,
						...
						const insertedIds = result.rows.map((r: { contact_id: number }) => r.contact_id);
						...
						if (skippedIds.length > 0) {
							...
							await pool.query(
								"DELETE FROM contacts_lists_links WHERE contact_id = ANY($1) AND list_id = $2",
								[skippedIds, listId],
							);
							...
							if (retriedCount < skippedIds.length) {
								const stillSkipped = skippedIds.filter(
									(id) => !retryResult.rows.find((r: { contact_id: number }) => r.contact_id === id),
								);
								logger.warn(
